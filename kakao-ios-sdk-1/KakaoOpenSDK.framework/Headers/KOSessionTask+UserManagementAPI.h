...
+ (instancetype)meTaskWithCompletionHandler:(nullable KOSessionTaskCompletionHandler)completionHandler DEPRECATED_MSG_ATTRIBUTE("전화번호 로그인이 오픈되면서 이메일이 없는 카카오계정이 존재할 수 있습니다. 다양한 상황에 대처할 수 있는 /v2/user/me가 적용된 userMeTaskWithCompletion: 메소드를 사용해주세요.");
+ (instancetype)meTaskWithSecureResource:(BOOL)secureResource
                       completionHandler:(nullable KOSessionTaskCompletionHandler)completionHandler DEPRECATED_MSG_ATTRIBUTE("전화번호 로그인이 오픈되면서 이메일이 없는 계정이 존재할 수 있습니다. 다양한 상황에 대처할 수 있는 /v2/user/me가 적용된 userMeTaskWithCompletion: 메소드를 사용해주세요.");
+ (instancetype)meTaskWithPropertyKeys:(nullable NSArray<NSString *> *)propertyKeys
                     completionHandler:(nullable KOSessionTaskCompletionHandler)completionHandler DEPRECATED_MSG_ATTRIBUTE("전화번호 로그인이 오픈되면서 이메일이 없는 계정이 존재할 수 있습니다. 다양한 상황에 대처할 수 있는 /v2/user/me가 적용된 userMeTaskWithCompletion: 메소드를 사용해주세요.");
+ (instancetype)meTaskWithSecureResource:(BOOL)secureResource
                            propertyKeys:(nullable NSArray<NSString *> *)propertyKeys
                       completionHandler:(nullable KOSessionTaskCompletionHandler)completionHandler DEPRECATED_MSG_ATTRIBUTE("전화번호 로그인이 오픈되면서 이메일이 없는 계정이 존재할 수 있습니다. 다양한 상황에 대처할 수 있는 /v2/user/me가 적용된 userMeTaskWithCompletion: 메소드를 사용해주세요.");
